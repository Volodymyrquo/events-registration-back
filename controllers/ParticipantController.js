...
export const register=async (req,res)=>{
    try {
        ...
        await ParticipantModel.updateOne(
                {_id:user._id},{$push:{event: req.body.event}}
            )
        await EventModel.updateOne(
            {_id:req.body.event},{$push:{participant: user._id}}
        )
   ...
   await EventModel.updateOne(
    {_id:req.body.event},{$push:{participant: doc._id}}
)
...
export const getAll = async (req,res)=>{
    try {
        const participants = await ParticipantModel.find().populate('event').exec();
